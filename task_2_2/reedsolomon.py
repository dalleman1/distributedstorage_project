...
def get_file_for_repair(fragments_to_retrieve, file_size,
    ...
    for _ in range(len(fragments_to_retrieve)):
        ...
        # In this case we don't care about the received name, just the
        # data from the second frame
        symbols.append({
            "chunkname": result[0].decode('utf-8'),
            "data": bytearray(result[1])
...
def start_repair_process(files, repair_socket, repair_response_socket):
    ...
    for file in files:
        ...
        # We parse JSON into a python dictionary
        ...
        nodes_with_fragment = list(nodes.difference(nodes_with_fragment))

        # Perform the actual repair, if necessary
        if len(missing_fragments) > 0:
            # Check that enough fragments still remain able to repair
            ...
            file_data = get_file_for_repair(existing_fragments[:symbols], # only as many asnecessary
                                            file["size"],
                                            repair_socket,
            ...
            for missing_fragment in missing_fragments:
                ...
                # Select the appropiate Reed Solomon coefficient vector
                ...
                node_id = nodes_with_fragment[number_of_repaired_fragments]

                # Use the node_id as topic
